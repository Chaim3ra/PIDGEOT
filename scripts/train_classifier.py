...
def main() -> None:
    ...
    axes[1].plot(history["epoch"], history["val_top1"], label="top-1")
    axes[1].plot(history["epoch"], history["val_top5"], label="top-5")
    axes[1].set_title("val accuracy"); axes[1].legend(); axes[1].grid(True, alpha=0.3)
    axes[2].plot(history["epoch"], history["val_median_km"])
    axes[2].set_title("val median km error"); axes[2].grid(True, alpha=0.3)
    fig.tight_layout()
